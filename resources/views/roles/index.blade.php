<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="text-2xl font-bold text-slate-950">Roles</h1>
        </div>
    </x-slot>

    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <form class="flex gap-2" method="GET">
            <input name="search" value="{{ request('search') }}" class="rounded-lg border-slate-300 text-sm shadow-sm focus:border-slate-500 focus:ring-slate-500" placeholder="Search roles">
        ...
        <a href="{{ route('roles.create') }}" class="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">Create Role</a>
    </div>
                    ...
                    <tr class="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('roles.edit', $role) }}" class="rounded-lg border border-slate-200 px-3 py-2 font-semibold hover:bg-white">Edit</a>
